...
export default class SingleStepGuess extends BaseTactic {
    ...
    find() {
        ...
        for (const { row, col } of emptyCells) {
            ...
            for (const candidate of cands) {
                ...
                if (cloned.updateCandidates)
                    cloned.updateCandidates();
                const contradiction = this.detectImmediateContradiction(cloned);
                if (contradiction) {
                    if (this.board.removeCandidate)
                        this.board.removeCandidate(row, col, candidate);
    ...
    detectImmediateContradiction(testBoard) {
        for (let r = 0; r < 9; r++) {
            for (let c = 0; c < 9; c++) {
                if (!testBoard.getValue(r, c) && testBoard.getCandidates(r, c).length === 0)
                    return { type: 'empty-cell', row: r, col: c };
            }
        }
        for (let r = 0; r < 9; r++) {
            for (let d = 1; d <= 9; d++) {
                if (!testBoard.rowHasValue(r, d) && testBoard.countCandidatePlacesInRow(r, d) === 0)
                    return { type: 'row-no-place', row: r, digit: d };
            }
        }
        for (let c = 0; c < 9; c++) {
            for (let d = 1; d <= 9; d++) {
                if (!testBoard.colHasValue(c, d) && testBoard.countCandidatePlacesInCol(c, d) === 0)
                    return { type: 'col-no-place', col: c, digit: d };
            }
        }
        for (let br = 0; br < 3; br++) {
            for (let bc = 0; bc < 3; bc++) {
                for (let d = 1; d <= 9; d++) {
                    if (!testBoard.boxHasValue(br, bc, d) && testBoard.countCandidatePlacesInBox(br, bc, d) === 0)
                        return { type: 'box-no-place', boxRow: br, boxCol: bc, digit: d };
                }
            }
        }
        return null;
