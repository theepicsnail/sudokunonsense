...
export function getHiddenSinglesFromBoard(board: any): Array<{ row: number; col: number; value: number; type: string }> {
    const singles: Array<{ row: number; col: number; value: number; type: string }> = [];
...
export function getRowCandidates(board: any, row: number): Record<number, number[]> {
    const candidates: Record<number, number[]> = {} as any;
...
export function getColumnCandidates(board: any, col: number): Record<number, number[]> {
    const candidates: Record<number, number[]> = {} as any;
    for (let value = 1; value <= 9; value++) candidates[value] = [];
    for (let row = 0; row < 9; row++) {
        if (board.board[row][col] === 0) {
...
export function getBoxCandidates(board: any, boxRow: number, boxCol: number): Record<number, { row: number; col: number }[]> {
    const candidates: Record<number, { row: number; col: number }[]> = {} as any;
    for (let value = 1; value <= 9; value++) candidates[value] = [];
