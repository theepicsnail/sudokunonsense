...
class SudokuApp {
    ...
    // DOM references
    puzzleGrid: HTMLElement | null = null;
    tacticSelect: HTMLSelectElement | null = null;
    tacticDescription: HTMLElement | null = null;
    solvingLog: HTMLElement | null = null;
    extensionInfo: HTMLElement | null = null;
    cells: HTMLElement[] = [];

    initializeUI() {
        this.puzzleGrid = document.getElementById('puzzleGrid') as HTMLElement;
        this.tacticSelect = document.getElementById('tacticSelect') as HTMLSelectElement;
        this.tacticDescription = document.getElementById('tacticDescription') as HTMLElement;
        this.solvingLog = document.getElementById('solvingLog') as HTMLElement;
        this.extensionInfo = document.getElementById('extensionInfo') as HTMLElement;
    ...
    createSudokuGrid() {
        ...
        for (let row = 0; row < 9; row++) {
            for (let col = 0; col < 9; col++) {
                ...
                input.maxLength = 1;
                (input as HTMLInputElement).inputMode = 'numeric';
                ...
                input.addEventListener('input', (e) => this.handleCellInput(e as InputEvent & { target: HTMLInputElement }, row, col));
                input.addEventListener('keydown', (e) => this.handleCellKeydown(e as KeyboardEvent, row, col));
    ...
    handleCellKeydown(event: KeyboardEvent, row: number, col: number) {
    ...
    handleCellFocus(row: number, col: number) { this.highlightRelatedCells(row, col); }
    handleCellBlur(row: number, col: number) { this.clearHighlights(); }

    highlightRelatedCells(row: number, col: number) {
        for (let r = 0; r < 9; r++) {
            for (let c = 0; c < 9; c++) {
    ...
    getCellElement(row: number, col: number) { return this.cells[row * 9 + col]; }
    ...
    renderCandidatesForCell(row: number, col: number) {
        const cell = this.getCellElement(row, col);
        const overlay = cell.querySelector('.candidates') as HTMLElement;
        const value = this.board.getValue(row, col);
        const spans = overlay.children;

        if (value !== 0) {
            for (let i = 0; i < 9; i++) (spans[i] as HTMLElement).textContent = '';
            return;
        }
        ...
        for (let n = 1; n <= 9; n++) {
            const idx = n - 1;
            (spans[idx] as HTMLElement).textContent = candidates.includes(n) ? String(n) : '';
        }
    }

    clearBoard() {
        this.board = new (this.board.constructor as any)();
    ...
    executeStep() {
        ...
        if (result && result.found) {
            ...
            if (this.board.isComplete && this.board.isComplete()) { this.logMessage('Puzzle solved!', 'success'); this.stopAutoSolve(); }
        } else {
    ...
    startAutoSolve() {
        ...
        const btn = document.getElementById('autoSolveBtn') as HTMLElement | null;
        if (btn) { btn.textContent = 'Stop Auto Solve'; btn.classList.remove('btn-success'); btn.classList.add('btn-warning'); }
    ...
    stopAutoSolve() {
        this.autoSolving = false;
        const btn = document.getElementById('autoSolveBtn') as HTMLElement | null;
        if (btn) { btn.textContent = 'Auto Solve'; btn.classList.remove('btn-warning'); btn.classList.add('btn-success'); }
    ...
    checkConflicts() {
        ...
        const noCandKeys = new Set<string>();
        const unitIssueKeys = new Set<string>();
    ...
    setsEqual(a: Set<string>, b: Set<string>) {
        if (a.size !== b.size) return false;
        for (const v of a) if (!b.has(v)) return false;
        return true;
    }

    logMessage(message: string, type: string = 'info') {
