...
export class SudokuTactics {
    ...
    executeTactic(tacticName) {
        ...
        if (!Klass)
            return { found: false, message: `Unknown tactic: ${tacticName}` };
        const tactic = new Klass(this.board, this.extensions);
        if (typeof tactic.setExtensions === 'function')
            tactic.setExtensions(this.extensions);
        if (typeof tactic.find === 'function')
            return tactic.find();
