...
export class BoxLineReductionStrategy {
    apply(board) {
        // For each digit
...
export class PointingPairsStrategy {
    apply(board) {
        // For each digit
        for (let digit = 1; digit <= 9; digit++) {
            // For each box
            for (let boxRow = 0; boxRow < 3; boxRow++) {
                for (let boxCol = 0; boxCol < 3; boxCol++) {
                    ...
                    if (positions.length < 2)
                        continue; // Only applies for pairs/triples
...
export class NakedTriplesStrategy {
    apply(board) {
...
export class NakedPairsStrategy {
    apply(board) {
        // Helper to check a unit (array of [row, col])
        function checkUnit(cells) {
            const pairs = [];
            ...
            for (const entry of pairs) {
                if (entry.positions.length === 2) {
                    for (const [row, col] of cells) {
                        ...
                        if (entry.positions.some(([r, c]) => r === row && c === col))
                            continue;
...
export class HiddenSinglesStrategy {
    apply(board) {
        // Check rows
        for (let row = 0; row < 9; row++) {
            for (let digit = 1; digit <= 9; digit++) {
...
export class BasicStrategy {
    ...
    apply(board) {
        for (let row = 0; row < 9; row++) {
            for (let col = 0; col < 9; col++) {
                const candidates = board.getCandidates(row, col);
                if (candidates.size <= 1 || board.isFrozen(row, col))
                    continue;
